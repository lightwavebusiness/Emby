...
def getToken():
	return jsonRead('http://ws.audioscrobbler.com/2.0/?method=auth.gettoken&format=json&api_key='+key)['token']
	...
	if "lastfm" in sources.keys():
		if autocomplete:
			...
			for catagory in sources["lastfm"]:
				url = "http://ws.audioscrobbler.com/2.0/?method=%s.search&format=json&page=%s&limit=%s&api_key=%s&%s=%s&token=%s" % (
					catagory, 
					str(page),
					str(limit), 
					key,
...
def getHits():
	hits = []
	url = "http://ws.audioscrobbler.com/2.0/?method=chart.getlovedtracks&api_key=%s&token=%s&format=json" % (
		key,
		getToken()
	)
...
def getAlbum(album, artist):
    data = []  
    album = jsonRead("http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=" + key + "&artist=" + artist + "&album=" + album + "&token=" + getToken() + "&format=json")
    try:
        tracks = album["album"]["tracks"]["track"]
        for track in tracks:
            name = track["name"]
