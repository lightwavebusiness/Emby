...
import ast
from emby import search, songURL, jsonRead, bingImage, getHits
import emby
...
def getAlbum():
    ...
    #When displaying in search, hide image, otherwise, show album image

    data = []  

    print emby.key, artist, album

    album = jsonRead("http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=" + emby.key + "&artist=" + artist + "&album=" + album + "&format=json")
    #print "http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=" + emby.key + "&artist=" + artist + "&album=" + album + "&format=json"
